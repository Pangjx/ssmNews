...
public class NewsController {
    ...
    public Msg checknews(@RequestParam("newsName")String newsName){
        //数据库用户名重复校验
        boolean b = newsService.checkUser(newsName);
        if(b){
            return Msg.success();
        }else{
    ...
    @ResponseBody
    public Msg saveNews(@Valid news news,BindingResult result) throws ParseException {
            System.out.println(news);
              if(result.hasErrors()){
    ...
    public Msg deletenews(@PathVariable("ids")String ids){
        ...
        if(ids.contains("-")){
            ...
            newsService.deleteBatch(del_ids);
        }else{
    ...
    public Msg savenews(news news, HttpServletRequest request){
        System.out.println("请求体中的值："+request.getParameter("newstitle"));
        System.out.println("将要更新的员工数据："+news);
